...
const navLinkClass =
  "relative flex h-10 items-center px-[1.15rem] text-[0.82rem] font-normal text-[#f8f6f4] transition-all duration-500 before:absolute before:left-1/2 before:top-[calc(100%+0.2rem)] before:h-1 before:w-1 before:-translate-x-1/2 before:rounded-full before:bg-white before:opacity-0 before:transition-opacity before:duration-500 hover:before:opacity-100";

function BrandMark() {
  return (
    <div className="flex flex-col items-center text-[#f8f6f4]">
      <span className="mb-1 text-[0.72rem] font-normal uppercase tracking-[0.6em]">The</span>
...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className={`fixed inset-x-0 top-0 z-40 border-b px-4 transition-all duration-500 md:px-8 xl:px-12 ${
          scrolled
            ? "border-white/15 bg-[rgba(0,0,0,0.24)] backdrop-blur-[10px]"
            : "border-white/18 bg-transparent"
        }`}
      >
        <div className="flex h-[92px] w-full items-center justify-between">
          ...
          <a href="/" aria-label="The Damai" className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            <BrandMark />
          </a>

          <div className="flex items-center">
            <a
              className="hidden min-h-[3.8rem] items-center justify-center rounded-full bg-white/20 px-8 text-[0.82rem] font-normal text-[#f8f6f4] backdrop-blur-[14px] transition-all duration-500 hover:-translate-y-0.5 hover:bg-white/28 lg:inline-flex"
              href="#packages"
            >
              Book your stay
            </a>
              ...
              <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
                <div>
                  ...
                  <div className="grid gap-3 text-[1rem] leading-none text-[rgba(50,32,24,0.76)]">
                    <a href="#villas" onClick={() => setMenuOpen(false)}>
                      Garden Villa
                    </a>
                    <a href="#villas" onClick={() => setMenuOpen(false)}>
                      Pool Villa
