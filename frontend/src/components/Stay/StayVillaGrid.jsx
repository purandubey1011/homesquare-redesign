import { villaPairs } from "./stayData";
...
function VillaCard({ villa }) {
  return (
    <article className="text-center" id={villa.id}>
      <div className="relative overflow-hidden bg-[#d8d0c8]">
        <img
          className="h-[32rem] w-full object-cover md:h-[42rem] lg:h-[48rem]"
          src={villa.image}
          alt={villa.name}
          loading="lazy"
        />
      ...
      <h2 className="mt-10 font-['Kaftan_Serif'] text-[clamp(3rem,4.6vw,4.35rem)] leading-[0.94] tracking-[-0.04em] text-[#34211a]">
        {villa.name}
      </h2>
      <p className="mx-auto mt-5 max-w-[32rem] text-[0.98rem] leading-[1.5] text-[rgba(72,54,42,0.8)] md:text-[1rem]">
        {villa.description}
      </p>
    </article>
  );
...
export default function StayVillaGrid() {
  return (
    <section className="bg-[#eae5df] px-4 pb-24 md:px-8 lg:px-10 lg:pb-28">
      <div className="mx-auto max-w-[1220px] space-y-20 lg:space-y-24">
        {villaPairs.map((pair) => (
          <div key={pair[0].id} className="grid grid-cols-1 gap-x-10 gap-y-16 lg:grid-cols-2">
            {pair.map((villa) => (
