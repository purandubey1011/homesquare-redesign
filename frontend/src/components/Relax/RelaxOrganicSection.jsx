import { organicSection } from "./relaxData";
...
export default function RelaxOrganicSection() {
  return (
    <section className="bg-[#eae5df] px-0 pb-32 pt-3 md:pb-40 md:pt-4">
      <div className="mx-auto grid max-w-[1920px] grid-cols-1 gap-6 md:grid-cols-3 md:gap-8">
        {organicSection.galleryImages.map((image) => (
          <div key={image.src} className="overflow-hidden">
            <img
              className="h-[24rem] w-full object-cover md:h-[38rem] lg:h-[40rem]"
              src={image.src}
              alt={image.alt}
              loading="lazy"
            />
      ...
      <div className="mx-auto mt-28 flex max-w-[58rem] flex-col items-center px-4 text-center md:mt-36 md:px-8">
        <Ornament />
        <h2 className="mt-7 font-['Kaftan_Serif'] text-[clamp(3.4rem,5.9vw,5.8rem)] leading-[0.92] tracking-[-0.05em] text-[#34211a]">
          {organicSection.title}
        </h2>
        <p className="mt-10 max-w-[44rem] font-extralight text-[clamp(1rem,1.15vw,1.08rem)] leading-[1.52] text-[rgba(72,54,42,0.82)]">
          {organicSection.description}
        </p>
        <div className="mt-20 overflow-hidden">
          <img
            className="h-[24rem] w-[16rem] object-cover md:h-[23rem] md:w-[16rem]"
            src={organicSection.image}
            alt={organicSection.title}
            loading="lazy"
          />
        </div>
      </div>
