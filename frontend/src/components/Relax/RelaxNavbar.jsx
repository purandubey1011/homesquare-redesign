...
import { discoverLinks, relaxNavLinks, stayLinks } from "./relaxData";
...
export default function RelaxNavbar() {
    ...
    <>
      <header
        className={`fixed inset-x-0 top-0 z-40 border-b px-4 transition-all duration-500 md:px-8 xl:px-12 ${
          scrolled
            ? "border-white/15 bg-[rgba(0,0,0,0.28)] backdrop-blur-[10px]"
        ...
        <div className="flex h-[92px] w-full items-center justify-between">
          <div className="flex min-w-0 items-center gap-4 xl:gap-6">
            ...
            <nav className="hidden items-center lg:flex" aria-label="Primary">
              {relaxNavLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className={`${navLinkClass} ${
                    link.label === "Relax" ? "before:opacity-100" : ""
                  }`}
                >
                  {link.label}
                </a>
