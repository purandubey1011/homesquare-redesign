import { packageCards, packagePageHero } from "./packagesData";

function Ornament({ className = "h-5 w-5", fill = "rgba(255,255,255,0.92)" }) {
  return (
    <svg
      aria-hidden="true"
      className={className}
...
function PackageCard({ item }) {
  return (
    <article className="group relative h-full min-h-[30rem] overflow-hidden bg-[#1b1614] text-[#f6f0ea] sm:min-h-[34rem]">
      <img
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-between px-6 py-8 text-center sm:px-8 sm:py-10">
        <span className="mt-1 text-[0.88rem] font-medium uppercase tracking-[0.08em] text-[rgba(246,240,234,0.76)] sm:text-[0.95rem]">
          {item.tag}
        </span>

        <div className="flex max-w-[28rem] flex-col items-center">
          <h3 className="m-0 font-['Kaftan_Serif'] text-[clamp(2.7rem,4vw,4rem)] leading-[0.98] tracking-[-0.04em] text-white">
            {item.name}
          </h3>
          <p className="mt-5 max-w-[24rem] text-[1rem] leading-[1.45] text-[rgba(246,240,234,0.86)] sm:text-[1.04rem]">
            {item.description}
          </p>
        </div>

        <a
          className="inline-flex min-h-[3.6rem] items-center justify-center gap-4 rounded-full bg-[rgba(108,108,112,0.72)] px-8 text-[0.98rem] font-semibold text-white backdrop-blur-[14px] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[rgba(122,122,126,0.86)]"
          href="/#packages"
        >
          <Ornament className="h-[1.05rem] w-[1.05rem]" />
          <span>More info</span>
          <Ornament className="h-[1.05rem] w-[1.05rem]" />
        </a>
      </div>
...
export default function PackagesHeroSection() {
  return (
    <section className="bg-[#eae5df]">
      <div className="px-4 pb-18 pt-[14.75rem] text-center md:px-8 md:pb-22 md:pt-[17rem]">
        <div className="mx-auto max-w-[58rem]">
          <span className="text-[0.95rem] uppercase tracking-[0.16em] text-[#0b7178]">
            {packagePageHero.eyebrow}
          </span>
          <h1 className="mt-8 font-['Kaftan_Serif'] text-[clamp(4.35rem,7.1vw,7.05rem)] leading-[0.9] tracking-[-0.06em] text-[#34211a]">
            {packagePageHero.title}
          </h1>
          <p className="mx-auto mt-8 max-w-[46rem] text-[clamp(1.04rem,1.18vw,1.14rem)] leading-[1.55] text-[rgba(72,54,42,0.84)]">
            {packagePageHero.description}
          </p>
        </div>
      </div>

      <div className="px-4 pb-20 md:px-8 md:pb-24">
        <div className="mx-auto grid max-w-[1860px] grid-cols-1 gap-6 md:grid-cols-2 xl:h-[calc(100svh-106px-2.5rem)] xl:grid-cols-3 xl:gap-16">
